...
export function validateOp(op) {
  ...
  switch (op.type) {
    ...
    case 'pin.update': { if (!isId(op.id)) return 'bad id'; if (op.id === 'start') return 'start pin is fixed';
      ...
      for (const k of Object.keys(p)) if (!['name', 'checked', 'shore', 'x', 'z'].includes(k)) return 'bad patch key';
...
export function planOp(state, op) {
  ...
  switch (op.type) {
    ...
    case 'pin.add': { const p = { id: op.pin.id, x: op.pin.x, z: op.pin.z, type: op.pin.type, name: op.pin.name, checked: op.pin.checked, ...(op.pin.shore ? { shore: true } : {}), ...(op.pin.log ? { log: op.pin.log } : {}), ...(op.pin.sightings?.length ? { sightings: op.pin.sightings } : {}) }; persist.push({ table: 'pins', id: p.id, json: JSON.stringify(p) }); break; }
...
export function applyOp(state, op) {
  ...
  switch (op.type) {
    ...
    case 'pin.add': { const p = { id: op.pin.id, x: op.pin.x, z: op.pin.z, type: op.pin.type, name: op.pin.name, checked: op.pin.checked, ...(op.pin.shore ? { shore: true } : {}), ...(op.pin.log ? { log: op.pin.log } : {}), ...(op.pin.sightings?.length ? { sightings: op.pin.sightings } : {}) }; state.pins.set(p.id, p); break; }
