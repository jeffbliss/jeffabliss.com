...
- Leg log (dead reckoning): from a chosen pin, enter legs as bearing + seconds + gait (e.g. `NE 40 jog`); the app draws the path as ink, drops a pin at the end with an uncertainty circle that grows per leg, and the whole log is one undo step that syncs. Later: re-anchor a closed loop onto a known pin so the chain corrects together.
